...
import { MemoryRouter, Route } from 'react-router-dom'; // Import MemoryRouter and Route
...
describe('CasoTable Component', () => {
  ...
  test('renders Preloader when data is initially null', async() => {
    ...
    //ExamService.getExams.mockResolvedValue(new Promise(() => {})); // Never resolves for this test
    ExamService.getExams.mockImplementation(() => {
  ...
  test('data is re-loaded when page parameter changes', async () => {
    ...
    await waitFor(() => {
        expect(ExamService.getExams).toHaveBeenCalledWith(3);
  ...
  ExamService.getExams.mockReturnValue(initialPromise);
