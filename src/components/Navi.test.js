...
describe('Navi Component', () => {
  ...
  test('renders Navbar brand and static links correctly', () => {
    ...
    const casoClinicoLink = screen.getByRole('link', { name: /caso clinico/i });
  ...
  describe('Conditional "Salir" (Logout) Link', () => {
    test('renders "Salir" link when user is authenticated', () => {
      Auth.isUserAuthenticated.mockReturnValue(true);
      renderNavi();
      const salirLink = screen.getByRole('link', { name: /salir/i });
      expect(salirLink).toBeInTheDocument();
      expect(salirLink).toHaveAttribute('href', '/logout');
    });
