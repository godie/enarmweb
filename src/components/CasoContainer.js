...
class CasoContainer extends React.Component {
    ...
    for (var answer of answers) {
      ...
      if (answer.id > 0) {
        answers_attributes.push(
          Object.assign({}, { id: answer.id }, prepareAnswer)
        );
      } else {
  ...
  addAnswer(index, event) {
    ...
    const caso = this.state.caso;
    this.currentId =
      "answer-" + index + "-" + caso.questions[index].answers.length;
...
export default withRouter(CasoContainer);
