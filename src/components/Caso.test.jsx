...
// Al inicio de tu archivo de test
...
const renderCaso = (clinicCaseId = 1, props = {}) => {
  return render(
    <Caso clinicCaseId={clinicCaseId} {...props} />
  );
};
...
describe('Caso Component', () => {
  ...
  test('clicking "Siguiente" when all questions answered calls sendAnswers, then navigates on second click', async () => {
    ...
    expect(mockPush).not.toHaveBeenCalled(); // Not yet navigated
    ...
    await waitFor(() => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/caso/2'); 
    });
  });
