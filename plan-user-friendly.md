...
## Mejoras técnicas / Futuro

### Router y URLs sin almohadilla (#)

- **Estado actual (desde fix/restore-mejoras):** Se usa `BrowserRouter` de React Router y enlaces con `<Link to="/ruta">` (o prop `to` en CustomButton / CustomSideNavItem). Las URLs son limpias: `/login`, `/dashboard/casos/1`, etc., sin `#`.
- **Despliegue:** En producción el servidor debe devolver `index.html` para cualquier ruta (SPA fallback). Ejemplo nginx: `try_files $uri $uri/ /index.html;`. Vite en dev ya hace fallback por defecto.
- **Alternativas para el futuro (evaluar si se quiere más tipo-safety o convenciones):**
  - [TanStack Router](https://tanstack.com/router/latest): router tipo-first, carga de datos integrada, búsqueda tipada. Útil si el proyecto crece y se quiere menos magia en rutas.
  - Mantener React Router (react-router-dom v5/v6): ya cubre necesidades actuales; migrar a v6 implica cambios de API (ej. `<Switch>` → `<Routes>`, `component` → `element`).
